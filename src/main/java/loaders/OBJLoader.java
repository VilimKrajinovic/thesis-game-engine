...
public class OBJLoader {
    ...
    public static ModelData load(String fileName) {
        ...
        File objFile = new File(RESOURCES_PATH + fileName + ".obj");
        try {
            fileReader = new FileReader(objFile);
        } catch (FileNotFoundException e) {
            System.err.println("File not found in resources folder, don't use extensions when calling this method");
        }
    ...
    private static float convertDataToArrays(List<Vertex> vertices, List<Vector2f> textures, List<Vector3f> normals,
        ...
        for (int i = 0; i < vertices.size(); i++) {
            ...
            if (currentVertex.getLength() > furthestVector) {
                furthestVector = currentVertex.getLength();
            }
